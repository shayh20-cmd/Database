...
- **`data.disciplines`** holds 16 entries whose `id` equals their `label` (`ARCH`, `PM`, `TRAF`, `STRC`, `PLUM`, `ELEC`, …). They are user-editable, so the capture window must read them at runtime, never hardcode them.
